...
IMETA_VALUES = {'SIPPICAN': {'depths': [360.0, 600.0, 1000.0, 1350.0, 2000.0],
                             'models': [functools.partial(get_model_by_date, 'T4', 'T10',
                                                          datetime.datetime(year=1993, month=1, day=1)),
                                        lambda x: 'T4',
                                        functools.partial(get_model_by_date, 'T7', 'DEEP BLUE',
                                                          datetime.datetime(year=1997, month=1, day=1)),
                                        functools.partial(get_model_by_date, 'T5', 'FAST DEEP',
                                                          datetime.datetime(year=2007, month=1, day=1)),
                                        lambda x: 'T5',
                                        ],
                             },
                'TSK - TSURUMI SEIKI Co.': {'depths': [600.0, 1000.0, 2000.0],
                                            'models': [functools.partial(get_model_by_date, 'T4', 'T6',
                                                                         datetime.datetime(year=1995, month=1, day=1)),
                                                       functools.partial(get_model_by_date, 'T5', 'T7',
                                                                         datetime.datetime(year=1979, month=1, day=1)),
                                                       lambda x: 'T5',
                                                       ]
                                            }
                }
...
def generate_imeta():
    ...
    xbt_full = xbt.dataset.XbtDataset(user_args.input_path,
                                      date_range)
    # generate encoders for getting ML formatted data
    _ = xbt_full.get_ml_dataset(return_data=False)
    _ = xbt_full.filter_features(xbt.dataset.TARGET_FEATURES).encode_target(return_data=False)
    ...
    for year in range(user_args.start_year, user_args.end_year):
        ...
        (im_pr_instr, im_rec_instr, im_f1_instr, im_sup_instr) = sklearn.metrics.precision_recall_fscore_support(
            xbt_instr1, y_imeta_instr, average='micro')
