...
def process_xbt_file(path_nc, out_path=None):
    ...
    nc_data_dict['temperature_profile'] = [ds_nc1.variables['Temperature'][z_pos1:z_pos1 + zl1].data.tolist() for
                                           (z_pos1, zl1) in zip(z_pos, ds_nc1.variables['Temperature_row_size'])]
    nc_data_dict['temperature_quality_flag'] = [ds_nc1.variables['Temperature_IQUODflag'][z_pos1:z_pos1 + zl1] for
                                                (z_pos1, zl1) in zip(z_pos, ds_nc1.variables['Temperature_row_size'])]

    nc_data_dict['depth_profile'] = [ds_nc1.variables['z'][z_pos1:z_pos1 + zl1].data.tolist() for (z_pos1, zl1) in
                                     zip(z_pos, ds_nc1.variables['z_row_size'])]
    nc_data_dict['max_depth'] = [dp1[-1] for dp1 in nc_data_dict['depth_profile']]
    nc_data_dict['depth_quality_flag'] = [ds_nc1.variables['z_IQUODflag'][z_pos1:z_pos1 + zl1] for (z_pos1, zl1) in
                                          zip(z_pos, ds_nc1.variables['z_row_size'])]

    lut1 = {-9: 0, 1: 1}
    try:
        nc_data_dict['imeta_applied'] = [lut1[i1] for i1 in
                                         ds_nc1.variables['Temperature_Instrument_intelligentmetadata'][:].data]
    except KeyError:
...
def do_wod_extract(nc_dir, out_dir, fname_prefix, fname_suffix, temp_dir, start_year=None, end_year=None,
                   pool_size=DEFAULT_PREPROC_TASKS):
    start1 = time.time()
